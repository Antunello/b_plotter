...
std::vector<TString> get_track_grades(){

	std::vector<TString> grades;
...
//	init_d0sig_grades(light_d0sig_grades, "light");
	...
	std::vector<TString> grades = get_track_grades();
...
//
	...
	leg_std->Draw();
	//cust_multigraph->Draw("a");
	TFile *output_file = new TFile("IP3D_ROC.root","update");
	ROC_graph->Write();
	c_ROC_graph->Write();
	output_file->Close();
	TFile *output_sum = new TFile("IP3D_summary.root","recreate");
	ROC_graph->Write();
	c_ROC_graph->Write();
